...
ledger. The output shape is a simple list of ``KPIValue`` records.

KPIs are registered in ``KPI_REGISTRY``; the dashboard service looks
them up by name when rendering a ``WidgetType.KPI`` widget.
"""
...
def cash_balance(*, tenant_id: str, as_of: dt.date) -> list[KPIValue]:
    ...
    # Stub — real implementation queries accounting_engine_account.
