...
class TestReconciliationStates(TestCase):
    ...
    def test_br_split_001_state_transitions(self):
        ...
        from apps.accounting.models import ReconcileStatus
...
class TestReconciledBalance(TestCase):
    ...
    def test_br_split_004_reconciled_balance_excludes_not_cleared(self):
        ...
        # Lifecycle: draft -> lines -> post. The split under test is created
        # while the entry is still a draft, then the entry is posted.
        journal_entry = JournalEntryFactory(
            tenant=tenant,
            legal_entity=legal_entity,
            transaction_currency=currency,
    ...
    def test_br_split_004_cleared_balance_includes_both_cleared_and_reconciled(self):
        ...
        # Offsetting split, on its own account so it does not affect the
        # cleared balance of the account under test
        JournalLineFactory(
            journal_entry=journal_entry,
            account=offset_account,
