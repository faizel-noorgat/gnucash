...
**Indexing (always incremental, always summarized):**
- reindex changed files → `index_folder { "path": ".", "incremental": true, "use_ai_summaries": true }`
- embeddings are a **separate** pass — `index_folder` has no embedding switch → `embed_repo { "repo": "Gnucash/gnucash" }`
- never run a full re-index as routine hygiene; it is a deliberate act for a cold index or a parser upgrade
